...
class ChatService {
    ...
    async joinGroupRoom(roomId, user) {
        if (!roomId?.trim()) {
            throw new exceptions_1.BadRequestException("Missing roomId parameter");
        }
        const chat = await this.chatRepository.findOneChat({
            filter: {
                roomId,
    ...
    async sendGroupMessage({ roomId, content }, user) {
        ...
        if (!content?.trim()) {
            throw new exceptions_1.BadRequestException("Message content is required");
        }
    ...
    async createGroup({ participantsIds = [], group, }, file, user) {
        ...
        if (!groupName || groupName.length < 3) {
            throw new exceptions_1.BadRequestException("Group name must be at least 3 characters long");
        }
        if (!Array.isArray(participantsIds) || participantsIds.length === 0) {
            throw new exceptions_1.BadRequestException("At least one participant ID is required");
        }
        const normalizedParticipantIds = participantsIds
            .map((participantId) => {
            if (typeof participantId === "string") {
                if (!mongoose_1.Types.ObjectId.isValid(participantId)) {
                    throw new exceptions_1.BadRequestException(`Invalid participant id: ${participantId}`);
                }
            ...
            throw new exceptions_1.BadRequestException(`Invalid participant id: ${String(participantId)}`);
        })
        ...
        if (uniqueParticipantIds.length === 0) {
            throw new exceptions_1.BadRequestException("Group must include at least one other participant");
        }
        ...
        if (users.length !== uniqueParticipantIds.length) {
            throw new exceptions_1.BadRequestException("Some participants were not found or are not friends with the user");
        }
